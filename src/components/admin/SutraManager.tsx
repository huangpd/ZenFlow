...
import { Plus, Edit3, Trash2, ChevronLeft, Save, X } from 'lucide-react';
...
export default function SutraManager({ initialSutras }: { initialSutras: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // In a real app we'd re-fetch or use router.refresh(), but for now let's just reload the page or update local state optmistically if we returned the obj.
      // Since our action revalidates, a router.refresh() is best.
  ...
  const handleDelete = async (id: string) => {
    if (confirm('确定要删除这部经文吗？这将影响所有未完成的功课。')) {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-stone-800">经文库管理</h1>
          </div>
        ...
        <div className="grid gap-4">
          {sutras.map(sutra => (
            <div key={sutra.id} className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm hover:shadow-md transition-all group flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold text-stone-800 mb-1">{sutra.title}</h3>
        ...
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-stone-100 flex justify-between items-center bg-stone-50/50">
              <h3 className="text-xl font-bold text-stone-800">{editingSutra ? '编辑经文' : '新增经文'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-stone-400 tracking-widest uppercase ml-1">经文标题</label>
                  <input 
                    required 
